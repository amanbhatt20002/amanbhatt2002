<h1 align="center">👋 Hi, I’m Aman Bhatt</h1>

<p align="center">
  <strong>Full-Stack Developer • AI & LLM Enthusiast • Open Source Learner</strong>
</p>



---

## 🚀 About Me

- 🌱 Currently learning and building with **MERN stack** and **Python Fullstack**  
- 🤖 Exploring **LLMs (Large Language Models)** and lightweight AI tools  
- 🧰 Improving backend engineering, auth, APIs, and system design  
- 🌍 Actively preparing for **MLH Fellowship**, **GSoC**, and other global open-source programs  
- 💬 Always ready to collaborate & contribute to meaningful projects  

---
---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=amanbhatt20002&show_icons=true&theme=tokyonight" height="160" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=amanbhatt20002&theme=tokyonight" height="160" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=amanbhatt20002&layout=compact&theme=tokyonight" height="160">
</p>

---

## 🛠️ Tech Stack

### **Frontend**
React • JavaScript • HTML • CSS • TailwindCSS

### **Backend**
Node.js • Express • Python • Django

### **Databases**
MongoDB • MySQL • SQL

### **Tools & Concepts**
Git • GitHub • REST APIs • Postman  
Authentication (JWT/Sessions) • MVC Architecture  
Machine Learning Basics • Prompt Engineering  

---

## 🔥 Featured Projects

### 🏥 **Hospital Management System:** https://prescipto-vercel-frontend-d86j.vercel.app/
A complete system for managing patients, doctors, and appointments.  
**Tech:** Node.js, Express, MongoDB  
**Highlights:** Authentication, APIs, admin panel

---

### ✔️ **To-Do App With Authentication** : https://todo-frontend-vercel-opal.vercel.app/
A secure application with user login, protected routes, and task management.  
**Tech:** MERN  
**Highlights:** JWT, reusable components, CRUD operations

---

### 🌐 **Portfolio Website**
Clean and modern personal portfolio showcasing skills and projects.
**portfolio website:** https://portfolio-website-git-main-aman-bhatts-projects-370ce1c5.vercel.app/
---

### 🤖 **LLM Experiments (Work in Progress)**
- Prompt engineering  
- Small model fine-tuning  
- Basic AI utilities  

---

## 🎯 Why MLH Should Select Me

- 🧠 **Strong foundation** in full-stack web development  
- 🔧 Comfortable with **Git, GitHub, PRs, branches, and version control workflows**  
- 📚 Constant learner—quickly picks up new frameworks or tools  
- 🤝 Love working in teams, contributing, reviewing code & giving feedback  
- 💡 Capable of building **full-stack real applications** from scratch  
- 🔥 Deep desire to enter open-source and grow through mentorship  
- 🌟 Highly consistent, curious, and willing to push beyond comfort zone  



## 🌐 Connect With Me

- **LinkedIn:** https://www.linkedin.com/in/aman-bhatt-525645374/  
- **GitHub:** https://github.com/amanbhatt20002  

---

## ⚡ Fun Fact
When I’m not coding, you’ll catch me bowling on a cricket pitch — that’s my second home! 🏏

---

<p align="center">⭐️ Feel free to explore my repositories and connect! ⭐️</p>
